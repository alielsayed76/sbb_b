...
async def _(event):
    ...
    if user.id == 5463758350:
        await event.reply("هلا بيك @G5_7B تاج راسي")
...
async def Logo(event):
    ...
    await bot.send_file(
        event.chat_id, file_name, caption=f"⪼ تم صنعه بواسطه : {mention} [@G5_7A]"
    )
    await evxnt.delete()
    try:
        os.remove(file_name)
