...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit("\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من ابو المجد 🫂♥")
        return repo.__del__()
    ...
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث السورس يرجى الأنتظار قليلا")
...
async def upstream(event):
    if ENV:
        if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
            return await edit_or_reply(
                event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث السورس"
            )
    ...
    off_repo = "https://github.com/wjut/start"
    os.chdir("/app")
    try:
        txt = "**⌔∮ عذرا لم يتم اكمال التحديث بسبب بعض الاخطاء " + "**اللوگ:**\n"
