...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧᥴ.𝘳 𝘴ꪮꪊ𝘳ᥴꫀⵧⵧⵧⵧ⍣**\n[ََِِ𝗰.ًًٍٍ𝗥 ًًٍٍ𝗨ٍّّ𝘀ََِِ𝗲ًًٍٍ𝗥ًًٍٍ𝗕ُُ𝗼ٖٔ𝗧 🧸♥](https://t.me/G5_7A)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ببوت ابو المجد **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/G5_7A"),
                Button.url(
                    "المطور",
                    "t.me/G5_7B",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل بوت ابو المجد من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[ᥴ.𝘳 𝘴ꪮꪊ𝘳ᥴꫀ♥](https://t.me/G5_7A)",
